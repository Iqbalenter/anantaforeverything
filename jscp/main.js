...
`;
  ...
  setupPaymentMethodSelection() {
    const paymentMethodContainers = document.querySelectorAll(
  ...
  async loadUserVouchers() {
    ...
    try {
      const res = await fetch(
        `https://api.deargift.online/api/vouchers/saved/${uid}`,
      );
      ...
      voucherList.innerHTML = this.vouchers
        .map(
          (v, idx) => `
...
`,
  ...
  handleVoucherChange(e) {
    ...
    if (e.target.checked) {
      ...
      if (voucherResult) {
        voucherResult.innerHTML = t("voucherApplied", {
  ...
  calculatePrice(currentSettings) {
    ...
    // 1. Kiểm tra nhạc nền
    const isCustomMusic =
      currentSettings.music !== "./music/happy-birthday.mp3";
    if (isCustomMusic) {
      items.push({
    ...
    if (currentSettings.enableBook) {
      ...
      const totalPages = currentSettings.pages
        ? currentSettings.pages.length
        : 0;
      if (totalPages > 10) {
  ...
  updatePricing(currentSettings = null) {
    ...
    const settingsToUse =
      currentSettings || window.settings || this.defaultSettings;
    ...
    `;
      ...
      pricingDetails.innerHTML = pricing.items
        .map(
          (item) => `
    ...
    const finalPrice =
      pricing.finalPrice !== undefined ? pricing.finalPrice : pricing.total;
  ...
  async handleActionButton() {
    ...
    try {
      ...
      if (
        this.currentTotal > 0 &&
        this.currentTotal < 10000 &&
        selectedPaymentMethod !== "PAYPAL"
      ) {
        this.showNotification(
          "❌ Số tiền thanh toán tối thiểu là 10.000 VNĐ!",
          "error",
        );
  ...
  prepareCompleteSettings(settingsToUse) {
    const completeSettings = {
      music: settingsToUse.music || "./music/ThankGodIFoundYou.mp3",
      countdown: settingsToUse.countdown || 3,
      matrixText: settingsToUse.matrixText || "LOVE",
      matrixColor1: settingsToUse.matrixColor1 || "#ffb6c1",
      matrixColor2: settingsToUse.matrixColor2 || "#ffc0cb",
      sequence: settingsToUse.sequence || "LOVE|YOU|MY|CUTEE|ANANTA|❤",
  ...
  async applyVoucher(orderCode) {
    try {
      ...
      const voucherRes = await fetch(
        "https://api.deargift.online/api/vouchers/apply",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async processPayment(orderCode, amount, websiteId, paymentMethod = "PAYOS") {
    try {
      ...
      const paymentRes = await fetch(
        "https://api.deargift.online/api/payment/create",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
